...
class color:
	"""This Class is used to give a format to text on the terminal"""
	PURPLE = '\033[95m'
	CYAN = '\033[96m'
	DARKCYAN = '\033[36m'
	BLUE = '\033[94m'
...
def title(s):
	"""This function is used to give format to the title of each segment."""
	return color.UNDERLINE + color.BOLD + s + color.END


class Element(object):
	"""Element class is used as a super class that defines a general element resume."""
	def __init__(self,name,where,date_start,date_end):
		"""Constructor for Element class defines the attributes for the class"""
		self.name = name
		self.where = where
	...
	def printing(self):
		"""This method is used to give format and structure to each element before being printed on screen."""
		return color.BOLD + self.name + color.END + " " + str(self.date_start) + "-" + str(self.date_end) + "\n" + self.where

class Education(Element):
	"""The class Education is a Subclass of Element represents anything related to education""" 
	pass

class Experience(Element):
	"""The class Experience is a Subclass of Element represents any Work Experience"""
	def set_function(self,function):
		"""This method adds the attribute function to the class to describe the function executed on the job"""
		self.function = function

	def printing(self):
		"""This method is overriding the super class method printing, and is used to give a special format and structure to each element before being printed on screen."""
		return color.BOLD + self.where + color.END + " " + str(self.date_start.year) + "-" + str(self.date_end.year) + "\n" + self.name +": "+ self.function

class Project(Element):
	"""The class Project is a Subclass of Element it represents major projects"""
	def set_description(self,description):
		"""This method adds the attribute description to the class to describe the project"""
...
class Event(Element):
	"""The class Eevent is a subclass of Element it represents important events"""
	def set_description(self,description):
		"""This method adds the attribute description to the class to describe the project"""
		self.description = description
	def printing(self):
